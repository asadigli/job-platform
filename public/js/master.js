...
function apply_for_job(id){
    var res = document.getElementById("disp_m_res").value;
    if (res != 0) {
      ...
      $.ajax({
        ...
        error: function(result){
            $("#notalert").html("<div class='notify-danger'><span>"+result.failed+"</span><a onclick='closenot()'>X</a></div>");
        },
        success: function(result){
            $("#notalert").html("<div class='notify'><span>"+result.success+"</span><a onclick='closenot()'>X</a></div>");
        },
        complete: function(){
          $("#job_list").load(location.href+" #job_list>*","");
          $('#loading-image').hide();
          $('#apply_popup').modal('toggle');
          document.getElementById("loading").innerHTML = "";
        }
      });
    }else{
      if ($('#disp_m_res').val() != '') {$('#disp_m_res').css('box-shadow', '0px 0px 7px 1px rgba(255,0,0,0.7)');}else{$('#disp_m_res').css('box-shadow', '0px 0px 7px 1px rgba(28,196,247,0.5)');}
    }
}
function filter_jobs(){
      ...
      jQuery.ajax({
        ...
        error: function(result){
            ...
            $("#notalert").html("<div class='notify-danger'><span>"+result+"</span><a onclick='closenot()'>X</a></div>");
            console.log("error");
        },
        success: function(result){
            $("#job_list").load(location.href+" #job_list>*","");
...
$('#fileup').change(function() {
  var i = $(this).prev('#upbtn span').clone();
  var file = $('#fileup')[0].files[0].name;
  $(this).prev('#upbtn span').text(file);
});
function getdata(){
  console.log("ok");
}
$('#send_resume').submit(function(event) {
    event.preventDefault();
    var formData = new FormData($(this)[0]);
    $.ajax({
        url: '/apply-as-ur-user',
        type: 'POST',
        data: formData,
        success: function(result){
          $("#notalert").html("<div class='notify'><span>"+result.success+"</span><a onclick='closenot()'>X</a></div>");
...
function cancel_request(id,text,yes,no){
  $("#cancelrequest").html("<div class='modal-dialog'><div class='modal-content'><div class='modal-header'><button type='button' class='close' data-dismiss='modal'>&times;</button></div><div class='modal-body'><p>"+text+"</p></div><div class='modal-footer'><button type='button' class='btn btn-default' data-dismiss='modal'>"+no+"</button><button type='button' class='btn btn-primary' onclick='cancel_job_request("+id+")'>"+yes+"</button></div></div></div>");
}
function add_new_sn(btn){
  document.getElementById("add_sn").innerHTML = "<div class='input-group-prepend'><select class='type' id='socnet_site'><option value='facebook'>Facebook</option><option value='instagram'>Instagram</option><option value='linkedin'>LinkedIn</option><option value='google-plus'>Google Plus</option><option value='twitter'>Twitter</option></select></div><input type='text' class='form-control' id='socnet_url' placeholder='URL...'><button type='button' onclick='add_new_socnet()'>"+btn+"</button>";
...
function cancel_job_request(id){
  $.ajaxSetup({headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
    $.ajax({
        url: "/job-request/cancel/"+id,
        type: 'GET',
        dataType: "JSON",
        data: {"id": id },
        error: function(result){
        ...
        success: function (result){
            $("#notalert").html("<div class='notify"+result.type+"'><span>"+result.success+"</span><a onclick='closenot()'>X</a></div>");
            $('#cancelrequest').modal('toggle');
            $("#jobreqlist").load(location.href+" #jobreqlist>*","");
            console.log('success');
        },
    });
}
...
$("#contact_us_btn").click(function(event){
    ...
    var em = $('#email').val();
    console.log();
    if ($('#name').val() == '') {$('#name').css('box-shadow', '0px 0px 7px 1px rgba(255,0,0,0.3)');}else{$('#name').css('box-shadow', '0px 0px 7px 1px rgba(28,196,247,0.5)');}
    if (em == '' | isEmail(em) == false) {$('#email').css('box-shadow', '0px 0px 7px 1px rgba(255,0,0,0.3)');}else{$('#email').css('box-shadow', '0px 0px 7px 1px rgba(28,196,247,0.5)');}
    if ($('#message').val() == '') {$('#message').css('box-shadow', '0px 0px 7px 1px rgba(255,0,0,0.3)');}else{$('#message').css('box-shadow', '0px 0px 7px 1px rgba(28,196,247,0.5)');}
    if ($('#msg_subject').val() == '') {$('#msg_subject').css('box-shadow', '0px 0px 7px 1px rgba(255,0,0,0.3)');}else{$('#msg_subject').css('box-shadow', '0px 0px 7px 1px rgba(28,196,247,0.5)');}
    if ($('#name').val() != '' && $('#email').val() != '' && $('#message').val() != '' && $('#msg_subject').val() != '') {
      document.getElementById("loading").innerHTML = "<center><img id='loading-image' src='https://upload.wikimedia.org/wikipedia/commons/c/c7/Loading_2.gif' style='height:200px;display:none;'></center>"
        $('#loading-image').show();
        $('#cont_form').hide();
        $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
        $.ajax({
          ...
          complete: function(){
            $('#loading-image').hide();
            $('#cont_form').show();
            $("#error").addClass("help-block with-errors").html('completed');
          }
        });
    }
});
...
setInterval(function() {closenot();}, 30000);
var jb = 2;
function get_more_jobs(){
  document.getElementById("ldpart").innerHTML = "<img id='loading-image' src='https://upload.wikimedia.org/wikipedia/commons/c/c7/Loading_2.gif' style='height:150px;'>";
  jb = jb + 2;
  $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
  $.ajax({
    url: "/get-more-jobs",
    ...
    complete: function(){
      document.getElementById("ldpart").innerHTML = "";
    }
  });
  console.log(jb);
}
...
// function isAvaliableusername(us,resid){
//
// }
$('#username').on('input', function() {
  ...
  $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
  $.ajax({
    url: "/check-res-un-avaliability",
    method: 'POST',
    data: {username: us,res_id: resid,},
    success: function(result){
      if (result.case == 'notexist') {
        $('#username').css('box-shadow', '0px 0px 7px 1px rgba(28,196,247,0.5)').css('border','1px solid #1cc4f7');
      }else{
        $('#username').css('box-shadow', '0px 0px 7px 1px rgba(255,0,0,0.5)').css('border','1px solid rgba(255,0,0,0.5)');
      }
    },
  });
});
function editres_f(){
  if (isEmail($('#official_email').val()) == false) {$('#official_email').css('box-shadow', '0px 0px 7px 1px rgba(255,0,0,0.3)');}else{$('#official_email').css('box-shadow', '0px 0px 7px 1px rgba(28,196,247,0.5)');}
  var id = $('#res_id').val();
  if (isEmail($('#official_email').val()) == true && id != 0) {
      $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
      $('#loading-image').show();
      $.ajax({
        url: "/edit-resume/"+id,
        method: 'POST',
        data: {
          full_name: $('#full_name').val(),official_email: $('#official_email').val(),
          title: $('#title').val(),website: $('#website').val(),
          username: $('#username').val(),status: $('#status').val(),
          about: $('#about_me').val(),phone_number: $('#phone_number').val(),
...
function add_new_socnet(){
  if (isURL($('#socnet_url').val()) == false) {$('#socnet_url').css('box-shadow', '0px 0px 7px 1px rgba(255,0,0,0.3)');}else{$('#socnet_url').css('box-shadow', '0px 0px 7px 1px rgba(28,196,247,0.5)');}
    if ($('#socnet_site').val() != "" & $('#socnet_url').val() != "" & isURL($('#socnet_url').val()) & $('#res_id').val() != "") {
      $.ajaxSetup({ headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}});
      $('#loading-image').show();
      jQuery.ajax({
